...
export async function prefetchHomePageData(): Promise<HomePrefetch> {
    const [
        ...
        fetchPhimJson(URLS.featuredTv),
        fetchPhimJson(URLS.posterChieuRap),
        fetchPhimJson(URLS.posterPhimBo),
        fetchPhimJson(URLS.topPhimLe),
        fetchPhimJson(URLS.topPhimBo),
        fetchPhimJson(URLS.featuredAnime),
        fetchPhimJson(URLS.posterKinhDi),
    ...
    return {
        ...
        featuredTv: mapFeatured(parseV1Items(tvRaw)),
        posterChieuRap: sortAndSlicePosterRowMovies(parseV1Items(chieuRapRaw)),
        posterPhimBo: sortAndSlicePosterRowMovies(parseV1Items(phimBoRaw)),
        topPhimLe: mapTop(parseV1Items(topLeRaw)),
        topPhimBo: mapTop(parseV1Items(topBoRaw)),
        featuredAnime: mapFeatured(parseV1Items(animeRaw)),
        posterKinhDi: sortAndSlicePosterRowMovies(parseV1Items(kinhDiRaw)),
        posterHoatHinh: sortAndSlicePosterRowMovies(parseV1Items(hoatHinhRaw)),
    };
}
