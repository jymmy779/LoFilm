...
export default function SwiperNavButtons({ prevClassName, nextClassName, variant = "default" }: SwiperNavButtonsProps) {
    ...
    const baseStyles = isGhost 
        ? "xl:block hidden sw-button absolute top-[35%] -translate-y-1/2 z-40 text-white/30 hover:text-white transition-colors disabled:opacity-0 cursor-pointer"
        : "xl:flex hidden absolute top-1/2 -translate-y-[calc(50%+24px)] z-10 w-10 h-10 rounded-full items-center justify-center transition-all duration-300 disabled:opacity-0 disabled:pointer-events-none cursor-pointer shadow-lg";
    ...
    return (
        <>
            <button className={`${prevClassName} ${baseStyles} ${variantStyles} ${sizeStyles} ${isGhost ? "-left-6 lg:-left-12" : "left-0 md:-left-4 lg:-left-5"}`}>
            ...
            <button className={`${nextClassName} ${baseStyles} ${variantStyles} ${sizeStyles} ${isGhost ? "-right-6 lg:-right-12" : "right-0 md:-right-4 lg:-right-5"}`}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" width={iconSize} height={iconSize} fill="currentColor">
